...
def _svc(db: AsyncIOMotorDatabase = Depends(get_db)) -> ExportService:
...
async def export_csv(dataset_id: str, svc: ExportService = Depends(_svc)):
    ...
    return Response(data, media_type="text/csv", headers={"Content-Disposition": f"attachment; filename={dataset_id}.csv"})
...
async def export_json(dataset_id: str, svc: ExportService = Depends(_svc)):
    ...
    return Response(data, media_type="application/json", headers={"Content-Disposition": f"attachment; filename={dataset_id}.json"})
